...
export default async function ContentPage() {
  ...
  return (
    <AdminShell>
      <PageHeader title="Education content library" kicker="Human review required">Manage age-specific English health literacy materials.</PageHeader>
      ...
      <div className="grid grid-3" style={{ marginBottom: 16 }}>
        ...
        <div className="card kpi-card"><span className="muted">Age/language coverage</span><strong className="metric-value">3 age bands</strong><p className="muted">English is the default learning language.</p></div>
      </div>
      <div className="card">
        <table className="table">
          ...
          <tbody>{educationMaterials.slice(0, 14).map((material) => <tr key={material.id}><td>{material.title.split(":")[0]}</td><td>{material.topic}</td><td>{material.ageBand}</td><td>{material.language.toUpperCase()}</td><td>{labelFor[material.literacyDimension]}</td><td><span className="badge">{material.sourceStatus === "SAMPLE" ? "Sample" : material.sourceStatus}</span></td><td>Demo seed</td><td><Link href={`/api/demo-login?username=asha&next=/youth/library/${material.topic}?ageBand=${material.ageBand}`}>Preview as youth</Link></td></tr>)}</tbody>
        </table>
      </div>
    </AdminShell>
  );
